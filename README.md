...
25.03.2026 (14:15)
------------------
Недооформленные исправления

25.03.2026 (21:12)
------------------
1) Провел эксперименты с программой Wireshark (захват сетевых пакетов)
   (Скачайте Wireshark: https://www.wireshark.org/download.html)
   Последовательность действий:
   1.1) Убедитесь, что в клиенте есть строка:.pingInterval(15, TimeUnit.SECONDS)
   и на сервере есть строка:pingPeriod = Duration.ofSeconds(15)
   1.2) Устанавливаем Wireshark на ПК (практически все по дефолту)
   1.3) Мобильник подключаем через USB к ПК кабелем,
   убираем "Отладка по USB" (для разработчиков),
   на мобильнике же выставляем (при подключенном кабеле) опцию
   "USB-модем"
   1.4) Запускаем Wireshark, под надписью "Захват"
   кликаем ниже комбобокса в списка "Ethernet"
   и выставляем фильтр в комбобоксе "tcp port 16999",
   в меню программы выбирпем "Захват->Старт".
   Пока ничего не должно выводиться.
   1.5) Запускаем сервер в IntelliJ IDEA и
   потом клиент в Android Studio. Ждем пару минут....
   1.6) В итоге должны в окне захвата программы будут появляться
   строки о приходе/отправлении ping/pong сообщений,
   как от клиента к серверу, так и от сервера к клиенту
   через каждые 15 сек.
   1.7) Через пару минут нажимаем красный квадрат (или "Стоп" в меню)
   1.8) Выделяем по Ctrl+A все захваченные строки и жмем Ctrl+C
   для копирования в буфер обмена.
   1.9) Скопированные строки выглядят так:
   Как определить, кто шлет пинг:
   Например:
   №	Время	От кого	Кому	Тип
   -------------------------------
   Сервер шлет пинг клиенту (без [MASKED])
   7	15.025498600	192.168.10.202 → 192.168.10.1	WebSocket Ping
   Клиент шлет пинг серверу (с [MASKED])
   9	15.028750300	192.168.10.1 → 192.168.10.202	WebSocket Ping [MASKED]
   Сервер шлет понг клиенту (без [MASKED])
   10	15.030882700	192.168.10.202 → 192.168.10.1	WebSocket Pong	      
   Клиент шлет понг серверу (с [MASKED])
   11	15.033141900	192.168.10.1 → 192.168.10.202	WebSocket Pong [MASKED]
2) Убедился, что при нажатии Home на клиенте примерно через 7 секунд сервер рвет соединение.


26.03.2026 (12:33)
------------------
Сделал небольшой маникюр

26.03.2026 (17:46)
------------------
1) После добавления класса WebSocketService в клиенте, проверил сценарий: когда после коннекта 
   с сервером уводить клиента в фон, сервер не рвет соединение.
2) Проверил (для порядка кастомные пинг/понги) для этого же сценария, 
  после чего закомментил все связанное с этими кастомными пинг/понгами.

28.03.2026 (19:50)
------------------
Добавил миграцию БД: V3__Change_id_to_bigserial.sql (просто для примера) 

28.03.2026 (22:25)
------------------
1) Добавил файл GamersDao.kt
2) Добавил в файл libs.versions.toml строки:
...
3) Для создания юнит-тестов:
   3.1) проапдейтил build.gradle.kts, добавив туда:
      в dependices:
   ...
   3.2) Создал файл GamersDaoTest.kt
   3.3) Для запуска всех тестов из терминала: gradlew test (но надо выйти из IDEA)
